...
/* ─────────────────────────────────────────────────────────────
   RESPONSIVE BREAKPOINTS (used inline via JS)
───────────────────────────────────────────────────────────── */
const useWindowSize = () => {
...
const KitchenView = () => {
  ...
  const [showBottomSheet, setShowBottomSheet] = useState(false); // station filter bottom sheet
  ...
  /* ──────────────────────────────────────────────────────────────
     SIDEBAR COMPONENT (shared between desktop & mobile drawer)
  ────────────────────────────────────────────────────────────── */
  const SidebarContent = () => (
    <>
      <div style={rs.sidebarHeader}>
        ...
        {isMobile && (
          <button onClick={() => setSidebarOpen(false)} style={{ marginLeft: 'auto', background: 'none', border: 'none', color: '#888', cursor: 'pointer' }}>
            <X size={20} />
          </button>
        )}
      </div>
      ...
      <div style={rs.sidebarStack} className="no-scrollbar">
        ...
        {visibleCategories.map(cat => {
          ...
          return (
            <button key={cat._id} onClick={() => { setSelectedCategory(k); setShowMetricsDashboard(false); setSidebarOpen(false); }}
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
                <span onClick={(e) => { e.stopPropagation(); trigger86KillToggle(cat.name); }} style={rs.mini86} title="86 item">
                  <EyeOff size={10} />
                </span>
                <span style={{ ...rs.countBadge, background: sel ? '#0f1013' : '#1a1c23', color: sel ? '#d3bfa2' : '#8e94a4', border: sel ? '1px solid #d3bfa2' : '1px solid #232730' }}>
    ...
    <div style={rs.root}>
        ...
        {sidebarOpen && isMobile && (
          <>
            ...
            <motion.aside initial={{ x: -300 }} animate={{ x: 0 }} exit={{ x: -300 }}
              ...
              style={{ ...rs.sidebar, position: 'fixed', left: 0, top: 0, bottom: 0, zIndex: 100, borderRadius: '0 16px 16px 0' }}>
              <SidebarContent />
            </motion.aside>
          </>
        )}
      ...
      <header style={rs.header}>
        <div style={rs.brandCluster}>
          ...
          <div style={rs.logoBadge}><ChefHat color="#d3bfa2" size={isMobile ? 22 : 30} /></div>
          <div>
            <h1 style={{ ...rs.mainTitle, fontSize: isMobile ? '0.85rem' : '1.15rem' }}>
              PRATYEKSHA / <span style={{ color: '#d3bfa2' }}>KDS PRO</span>
            </h1>
            <div style={{ display: 'flex', alignItems: 'center', gap: 6, marginTop: 2 }}>
              <span style={isOnline ? rs.dotGold : rs.dotRed} />
              {!isMobile && (
                <p style={rs.subTitle}>{isOnline ? "TITANIUM V17.0 • ONLINE" : "⚠️ DISCONNECTED"}</p>
              )}
            </div>
          </div>
        </div>
        ...
        <div style={rs.actionCenter}>
          {/* Station pills — hidden on mobile (moved to bottom bar) */}
          {!isMobile && (
          ...
          {/* Voice */}
          <button onClick={toggleVoiceListener}
            ...
            {!isMobile && <span style={{ color: isListening ? '#d3bfa2' : '#fff', fontSize: '0.72rem', fontWeight: 700 }}>
          ...
          <button onClick={() => setIsAggregateView(v => !v)} style={rs.utilBtn}>
            ...
            {!isMobile && <span style={{ fontSize: '0.72rem', fontWeight: 700 }}>{isAggregateView ? "TICKETS" : "SUMMARY"}</span>}
          </button>
          ...
          <div style={rs.ticketBadge}>
            ...
            {!isMobile && <span style={{ color: '#1a1c23', fontSize: '0.55rem', fontWeight: 900, marginLeft: 5 }}>TICKETS</span>}
          </div>
        </div>
      </header>
      ...
      <AnimatePresence>
        {interceptedAlerts.map(alert => (
          <motion.div key={alert.id} initial={{ scale: 0.9, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} exit={{ opacity: 0 }}
            style={{ ...rs.alertBox, flexDirection: isMobile ? 'column' : 'row', gap: isMobile ? 10 : 30 }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
              <div style={{ background: 'rgba(0,0,0,0.2)', padding: 10, borderRadius: 10 }}><BellRing size={20} /></div>
              <div>
                <h3 style={{ margin: 0, fontSize: isMobile ? '0.9rem' : '1.1rem', fontWeight: 900 }}>ORDER CHANGE — TABLE {alert.tableNumber}</h3>
      ...
      <div style={rs.body}>
        ...
        {isDesktop && (
          <aside style={rs.sidebar}>
            <SidebarContent />
          </aside>
        )}

        {/* Tablet: horizontal category scroll bar */}
        {isTablet && !isAggregateView && !showMetricsDashboard && (
          <div style={rs.tabletCategoryBar} className="no-scrollbar">
            {[{ label: 'ALL', key: 'ALL' }, ...visibleCategories.map(c => ({ label: c.name, key: c.categoryId?.toLowerCase().trim() || '' }))].map(({ label, key }) => {
              const sel = selectedCategory === key;
              return (
                <button key={key} onClick={() => setSelectedCategory(key)}
                  style={{ ...rs.tabCatBtn, background: sel ? 'linear-gradient(135deg,#bda88a,#d3bfa2)' : '#191b22', color: sel ? '#0f1013' : '#8a909f', border: sel ? 'none' : '1px solid #252932' }}>
                  {label.toUpperCase()}
                </button>
              );
            })}
        ...
        <main style={rs.workspace} className="no-scrollbar">
          {showMetricsDashboard ? (
            <motion.div initial={{ opacity: 0, y: 15 }} animate={{ opacity: 1, y: 0 }} style={rs.metricsPanel}>
              ...
              <h2 style={{ margin: '16px 0 0', fontSize: isMobile ? '1.1rem' : '1.5rem', fontWeight: 900 }}>SPEED METRICS</h2>
              <div style={{ display: 'flex', gap: 20, flexWrap: 'wrap', justifyContent: 'center', marginTop: 30, width: '100%', maxWidth: 700 }}>
                {[
                  ...
                  <div key={label} style={rs.metricBox}>
                    <small style={{ fontSize: '0.65rem', fontWeight: 900, color: '#5c616e', letterSpacing: '1.5px', display: 'block', marginBottom: 12 }}>{label}</small>
                    <div style={{ fontSize: isMobile ? '2.5rem' : '3.5rem', fontWeight: 950, color, fontFamily: 'monospace', lineHeight: 1 }}>{value}</div>
                  </div>
                ))}
              </div>
              <button onClick={() => setShowMetricsDashboard(false)} style={rs.metricClose}>BACK TO TRACKER</button>
            </motion.div>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: isMobile ? 'repeat(2,1fr)' : 'repeat(auto-fill,minmax(180px,1fr))', gap: 14 }}>
              {Object.entries(aggregatedTotals).map(([name, qty]) => (
                <div key={name} style={rs.aggCard}>
                  <div style={{ fontSize: isMobile ? '2.5rem' : '3.2rem', fontWeight: 900, color: '#d3bfa2', fontFamily: 'monospace', lineHeight: 1 }}>{qty < 10 ? `0${qty}` : qty}</div>
                  <div style={{ fontSize: '0.72rem', color: '#7c8291', marginTop: 10, fontWeight: 800, textTransform: 'uppercase', lineHeight: 1.3 }}>{name}</div>
                </div>
              ))}
            </div>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', height: '100%', gap: 12 }}>
                ...
                <div style={{ display: 'flex', justifyContent: 'center', gap: 6, flexShrink: 0 }}>
                  {filteredOrders.map((_, i) => (
                    <div key={i} onClick={() => setMobileCardIndex(i)} style={{
                ...
                <div style={{ flex: 1, overflow: 'hidden', position: 'relative' }} {...swipeHandlers}>
                  <AnimatePresence mode="wait">
                    <motion.div key={filteredOrders[safeCardIndex]?._id}
                ...
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexShrink: 0 }}>
                  <button onClick={() => setMobileCardIndex(i => Math.max(0, i - 1))}
                  ...
                  <button onClick={() => setMobileCardIndex(i => Math.min(filteredOrders.length - 1, i + 1))}
            ...
            /* ── DESKTOP / TABLET GRID ── */
            <div style={{ display: 'grid', gridTemplateColumns: isTablet ? 'repeat(auto-fill,minmax(280px,1fr))' : 'repeat(auto-fill,minmax(310px,1fr))', gap: 18, alignContent: 'flex-start' }}>
      ...
      {isMobile && (
        <nav style={rs.mobileBottomBar}>
          {['ALL', 'DINEIN', 'PARCEL'].map(s => (
            <button key={s} onClick={() => setStationFilter(s)}
              style={{ ...rs.bottomBarBtn, color: stationFilter === s ? '#d3bfa2' : '#4a4f5c', borderTop: stationFilter === s ? '2px solid #d3bfa2' : '2px solid transparent' }}>
              {s === 'ALL' ? <Monitor size={18} /> : s === 'DINEIN' ? <UtensilsCrossed size={18} /> : <Package size={18} />}
              <span style={{ fontSize: '0.55rem', fontWeight: 800, marginTop: 2 }}>{s === 'ALL' ? 'ALL' : s === 'DINEIN' ? 'DINE-IN' : 'PARCEL'}</span>
            </button>
          ))}
          <button onClick={() => setIsAggregateView(v => !v)}
            style={{ ...rs.bottomBarBtn, color: isAggregateView ? '#d3bfa2' : '#4a4f5c', borderTop: isAggregateView ? '2px solid #d3bfa2' : '2px solid transparent' }}>
            <BarChart3 size={18} />
            <span style={{ fontSize: '0.55rem', fontWeight: 800, marginTop: 2 }}>SUMMARY</span>
          </button>
          <button onClick={() => setShowMetricsDashboard(v => !v)}
            style={{ ...rs.bottomBarBtn, color: showMetricsDashboard ? '#d3bfa2' : '#4a4f5c', borderTop: showMetricsDashboard ? '2px solid #d3bfa2' : '2px solid transparent' }}>
            <TrendingUp size={18} />
      ...
      {/* ── WAITER CALLS ── */}
      <div style={{ position: 'fixed', bottom: isMobile ? 80 : 30, right: isMobile ? 12 : 30, zIndex: 2000, display: 'flex', flexDirection: 'column', gap: 12, maxWidth: isMobile ? 'calc(100vw - 24px)' : 400 }}>
        <AnimatePresence>
          {waiterCalls.map(call => (
            <motion.div key={call.id} initial={{ x: 200, opacity: 0 }} animate={{ x: 0, opacity: 1 }} exit={{ x: 200, opacity: 0 }}
              style={{ background: 'linear-gradient(135deg,#bda88a,#d3bfa2)', color: '#0f1013', padding: isMobile ? '12px 16px' : '14px 24px', borderRadius: 14, display: 'flex', alignItems: 'center', gap: 14, fontSize: '0.85rem', fontWeight: 950, boxShadow: '0 12px 30px rgba(0,0,0,0.4)' }}>
              <div style={{ background: 'rgba(0,0,0,0.07)', padding: 8, borderRadius: 8 }}><BellRing size={18} color="#000" /></div>
              <div style={{ flex: 1 }}><b>TABLE {call.tableNumber}</b>: {call.reason?.toUpperCase()}</div>
              <X size={18} style={{ cursor: 'pointer' }} onClick={() => setWaiterCalls(prev => prev.filter(c => c.id !== call.id))} />
            </motion.div>
          ))}
        </AnimatePresence>
      </div>
...
const KDSOrderCard = ({
  ...
  socketInstance, isNonVegMode, tenantOnlyVeg, isMobile
}) => {
  ...
  return (
    <motion.div layout initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }}
      ...
      {/* Urgency bar */}
      <div style={{ position: 'absolute', top: 0, left: 0, right: 0, height: 4, background: urgency === 'low' ? '#1f222a' : urgency === 'medium' ? 'linear-gradient(90deg,#1f222a,#d3bfa2)' : '#d3bfa2' }} />

      {/* Header */}
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 12 }}>
        <div>
          <h2 style={{ fontSize: isMobile ? '1.8rem' : '2.1rem', margin: 0, fontWeight: 900, color: '#fff', letterSpacing: '-0.5px', lineHeight: 1 }}>
            {order.tableNumber?.toLowerCase() === 'takeaway'
              ? <span style={{ display: 'inline-flex', alignItems: 'center', gap: 8, color: '#d3bfa2' }}><Package size={isMobile ? 20 : 24} /> PARCEL</span>
              : `T-${order.tableNumber}`
            }
          </h2>
          <span style={{ fontSize: '0.6rem', color: '#5c616e', fontWeight: 800 }}>ID: {order._id.slice(-4).toUpperCase()}</span>
        </div>
        <div style={{ display: 'flex', alignItems: 'center', gap: 7, padding: '4px 10px', borderRadius: 8, border: `1px solid ${urgency !== 'low' ? '#d3bfa2' : '#272a33'}`, background: '#0d0e11' }}>
          <Clock size={14} color={urgency !== 'low' ? '#d3bfa2' : '#5c616e'} />
          <span style={{ fontFamily: 'JetBrains Mono, monospace', fontWeight: 900, fontSize: isMobile ? '0.9rem' : '1rem', color: urgency === 'high' ? '#d3bfa2' : '#fff' }}>
            {formatTime(seconds)}
          </span>
        </div>
      </div>
      ...
      <div style={{ flex: 1, overflowY: 'auto' }} className="custom-scroll">
        {order.items.map((item, idx) => {
          ...
          if (!modeMatch) return (
            <div key={idx} style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '10px 0', borderBottom: '1px solid #1c1f26', opacity: crossed ? 0.06 : 0.2, pointerEvents: 'none', filter: 'grayscale(1)' }}>
              <div style={{ width: 30, height: 30, display: 'flex', justifyContent: 'center', alignItems: 'center', borderRadius: 7, fontWeight: 900, color: '#d3bfa2', background: '#0d0e11', fontFamily: 'monospace' }}>{item.quantity}</div>
              <span style={{ fontSize: '0.85rem', color: '#444', fontWeight: 600 }}>{item.name}</span>
            </div>
          );

          return (
            <div key={idx} onClick={() => toggleItemCrossed(idx)}
              style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '10px 0', borderBottom: '1px solid #1c1f26', cursor: 'pointer', opacity: crossed ? 0.2 : 1, transition: '0.15s', WebkitTapHighlightColor: 'transparent' }}>
              <div style={{ width: 30, height: 30, display: 'flex', justifyContent: 'center', alignItems: 'center', borderRadius: 7, background: crossed ? '#0d0e11' : '#1e2129', fontWeight: 900, color: '#d3bfa2', flexShrink: 0, fontFamily: 'monospace' }}>
                {item.quantity}
              </div>
              <div style={{ flex: 1, minWidth: 0 }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: 6, flexWrap: 'wrap' }}>
                  <span style={{
                    fontSize: isMobile ? '0.9rem' : '0.95rem', fontWeight: 700, lineHeight: 1.3,
                    ...
                    padding: item.isChefSpecial ? '2px 7px' : 0, borderRadius: item.isChefSpecial ? 5 : 0
                  }}>
                    {item.isChefSpecial && <Sparkles size={11} style={{ display: 'inline', marginRight: 3 }} />}
                    {item.name}
                  </span>
                  {!tenantOnlyVeg && (
                    <div style={{ width: 10, height: 10, border: `1.5px solid ${isVeg ? '#4a7c3f' : '#8a3030'}`, borderRadius: 2, display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
                      {isVeg
                  ...
                  <div style={{ background: forceParcel ? 'rgba(211,191,162,0.06)' : 'rgba(255,255,255,0.03)', color: forceParcel ? '#d3bfa2' : '#a0a5b5', fontSize: '0.5rem', padding: '2px 5px', borderRadius: 4, border: `1px solid ${forceParcel ? 'rgba(211,191,162,0.12)' : '#232731'}`, fontWeight: 900, display: 'flex', alignItems: 'center', gap: 2 }}>
                    {forceParcel ? <Package size={9} /> : <UtensilsCrossed size={9} />}
                    {forceParcel ? 'PARCEL' : 'DINE-IN'}
                  </div>
                </div>
                <span style={{ fontSize: '0.6rem', fontWeight: 900, color: item.portion?.toLowerCase() === 'half' ? '#d3bfa2' : '#7c8291', display: 'block', marginTop: 2 }}>
                  {item.portion?.toUpperCase() || 'STANDARD'}
                </span>
                {item.suggestion && (
                  <div style={{ color: '#bda88a', fontSize: '0.65rem', marginTop: 5, display: 'flex', alignItems: 'center', gap: 4, background: 'rgba(211,191,162,0.04)', padding: '4px 8px', borderRadius: 6, border: '1px solid rgba(211,191,162,0.08)' }}>
                    <StickyNote size={9} />
      ...
      <button
        ...
        style={{
          width: '100%', padding: isMobile ? '16px' : '14px', borderRadius: 12,
          border: '1px solid rgba(211,191,162,0.3)', fontWeight: 900,
          fontSize: isMobile ? '0.9rem' : '0.8rem', cursor: 'pointer', marginTop: 12,
...
/* ─────────────────────────────────────────────────────────────
   RESPONSIVE STYLE TOKENS
───────────────────────────────────────────────────────────── */
const rs = {
  root: { position: 'fixed', inset: 0, display: 'flex', flexDirection: 'column', background: '#0d0e11', padding: '12px', gap: 10, overflow: 'hidden' },
  header: { display: 'flex', justifyContent: 'space-between', alignItems: 'center', background: '#13151a', padding: '12px 18px', borderRadius: 14, border: '1px solid #1f222a', flexShrink: 0, zIndex: 10, gap: 10 },
  brandCluster: { display: 'flex', alignItems: 'center', gap: 12 },
  logoBadge: { background: '#0d0e11', padding: '8px', borderRadius: 10, border: '1px solid #1f222a', flexShrink: 0 },
  ...
  subTitle: { color: '#5c616e', margin: 0, fontSize: '0.58rem', letterSpacing: '2px', fontWeight: 800 },
  actionCenter: { display: 'flex', alignItems: 'center', gap: 8, flexShrink: 0 },
  utilBtn: { background: '#191b22', border: '1px solid #252932', color: '#fff', padding: '9px 12px', borderRadius: 9, cursor: 'pointer', display: 'flex', alignItems: 'center', gap: 6, transition: '0.15s', flexShrink: 0, minHeight: 40 },
  iconBtn: { background: '#191b22', border: '1px solid #252932', padding: '9px', borderRadius: 9, cursor: 'pointer', display: 'flex', alignItems: 'center', flexShrink: 0 },
  ticketBadge: { background: 'linear-gradient(135deg,#bda88a,#d3bfa2)', padding: '7px 14px', borderRadius: 9, display: 'flex', alignItems: 'center', flexShrink: 0, minHeight: 40, justifyContent: 'center' },
  ticketNum: { color: '#0f1013', fontSize: '1.2rem', fontWeight: 950, fontFamily: 'JetBrains Mono, monospace' },
  stationCapsule: { display: 'flex', background: '#0d0e11', padding: 4, borderRadius: 10, border: '1px solid #1f222a' },
  capsule: { padding: '7px 12px', background: 'transparent', border: 'none', color: '#5c616e', fontSize: '0.63rem', fontWeight: 900, cursor: 'pointer', borderRadius: 7, whiteSpace: 'nowrap' },
  capsuleActive: { padding: '7px 12px', background: 'rgba(211,191,162,0.08)', border: 'none', color: '#d3bfa2', fontSize: '0.63rem', fontWeight: 900, borderRadius: 7, whiteSpace: 'nowrap' },

  marquee: { display: 'flex', alignItems: 'center', background: '#0a0a0c', border: '1px solid #191b22', padding: '10px 16px', borderRadius: 10, gap: 12, overflow: 'hidden', flexShrink: 0 },
  marqueeLabel: { fontSize: '0.63rem', fontWeight: 900, color: '#bda88a', letterSpacing: '1px', display: 'flex', alignItems: 'center', gap: 5, flexShrink: 0 },
  marqueeToken: { background: '#13151a', border: '1px solid #1f222a', padding: '4px 10px', borderRadius: 7, display: 'flex', gap: 6, alignItems: 'center' },

  alertBox: { background: '#541717', border: '1px solid #ff4d4d', padding: '16px 24px', borderRadius: 14, display: 'flex', alignItems: 'center', justifyContent: 'space-between', boxShadow: '0 10px 40px rgba(0,0,0,0.4)', flexShrink: 0 },

  body: { display: 'flex', flex: 1, gap: 14, overflow: 'hidden', minHeight: 0 },

  sidebar: { width: 250, background: '#13151a', border: '1px solid #1f222a', borderRadius: 14, display: 'flex', flexDirection: 'column', padding: '16px 12px', flexShrink: 0, height: '100%', overflowY: 'auto' },
  sidebarHeader: { display: 'flex', alignItems: 'center', gap: 7, fontSize: '0.62rem', fontWeight: 900, color: '#5c616e', letterSpacing: '1.5px', marginBottom: 14, borderBottom: '1px solid #1f222a', paddingBottom: 10 },
  sidebarStack: { display: 'flex', flexDirection: 'column', gap: 7, overflowY: 'auto', flex: 1 },
  sidebarNode: { display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '12px', background: '#171921', border: '1px solid #20242e', color: '#8e94a4', width: '100%', textAlign: 'left', borderRadius: 9, cursor: 'pointer', fontSize: '0.72rem', fontWeight: 700, transition: 'all 0.15s', minHeight: 44 },
  activeSidebarNode: { display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '12px', background: 'linear-gradient(135deg,#bda88a,#d3bfa2)', border: 'none', color: '#0f1013', width: '100%', textAlign: 'left', borderRadius: 9, fontSize: '0.72rem', fontWeight: 950, boxShadow: '0 4px 12px rgba(211,191,162,0.12)', minHeight: 44 },
  countBadge: { fontSize: '0.63rem', padding: '3px 7px', borderRadius: 6, fontWeight: 900, border: '1px solid #232730' },
  mini86: { padding: '3px', cursor: 'pointer', display: 'inline-flex', background: 'rgba(255,255,255,0.01)', border: '1px solid #232730', borderRadius: 5, color: '#444', transition: '0.2s' },
  vegToggleWrap: { display: 'flex', background: '#0d0e11', borderRadius: 9, border: '1px solid #1f222a', padding: 3, marginBottom: 12, flexShrink: 0 },
  vegBtn: { flex: 1, padding: '7px 5px', borderRadius: 7, border: 'none', cursor: 'pointer', fontSize: '0.62rem', fontWeight: 900, display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 5, background: 'transparent', color: '#555', transition: 'all 0.15s' },
  ...
  tabletCategoryBar: { display: 'flex', gap: 8, overflowX: 'auto', flexShrink: 0, paddingBottom: 4 },
  tabCatBtn: { padding: '8px 14px', borderRadius: 9, cursor: 'pointer', fontSize: '0.65rem', fontWeight: 900, whiteSpace: 'nowrap', transition: '0.15s' },

  workspace: { flex: 1, overflowY: 'auto', minWidth: 0 },

  metricsPanel: { background: '#13151a', border: '1px solid #1f222a', padding: '40px 20px', borderRadius: 18, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', minHeight: '100%' },
  metricBox: { background: '#0d0e11', border: '1px solid #1f222a', padding: '30px 20px', borderRadius: 14, flex: 1, minWidth: 240, textAlign: 'center' },
  metricClose: { marginTop: 30, background: 'transparent', border: '1px solid rgba(211,191,162,0.25)', color: '#d3bfa2', padding: '12px 28px', borderRadius: 10, fontSize: '0.78rem', fontWeight: 900, cursor: 'pointer', minHeight: 48 },

  aggCard: { background: '#13151a', padding: '24px 16px', borderRadius: 14, border: '1px solid #1f222a', textAlign: 'center' },

  mobileBottomBar: { display: 'flex', background: '#13151a', border: '1px solid #1f222a', borderRadius: 14, padding: '4px', flexShrink: 0, gap: 2 },
  bottomBarBtn: { flex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center', padding: '10px 4px', background: 'transparent', border: 'none', cursor: 'pointer', transition: '0.15s', borderRadius: 10, gap: 2 },

  navBtn: { display: 'flex', alignItems: 'center', gap: 5, background: '#191b22', border: '1px solid #252932', color: '#d3bfa2', padding: '10px 16px', borderRadius: 9, cursor: 'pointer', fontSize: '0.72rem', fontWeight: 800, minHeight: 44 },
};
