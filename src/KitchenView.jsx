...
import { ChefHat, Timer, Zap, Hourglass, BellRing, StickyNote, ChevronDown } from 'lucide-react';
  ...
  return (
    <div style={styles.kdsContainer}>
      ...
      <style>{`
        ...
        .order-card-glow { transition: border 0.3s ease; }
        .order-card-glow:hover { border-color: rgba(211, 191, 162, 0.5) !important; }
      `}</style>
    </div>
  );
};
  ...
  return (
    <motion.div
      ...
      animate={{
        ...
        borderColor: isNewest ? '#d3bfa2' : isLate ? 'rgba(211, 191, 162, 0.6)' : 'rgba(255,255,255,0.05)'
      }}
      exit={{ opacity: 0, scale: 0.8, filter: 'blur(10px)' }}
      className="order-card-glow"
      style={{
        ...styles.card,
        boxShadow: isNewest ? '0 10px 40px rgba(211, 191, 162, 0.15)' : 'none'
      }}>
      ...
      <div style={styles.cardHeader}>
        ...
        <div style={{ ...styles.timerBadge, backgroundColor: isLate ? 'rgba(211, 191, 162, 0.1)' : '#111' }}>
          {isLate ? <Hourglass size={14} color="#d3bfa2" /> : <Timer size={14} color="#555" />}
          <span style={{ color: isLate ? '#d3bfa2' : '#777' }}>{elapsed}m</span>
        </div>
      </div>
      ...
      <div ref={listRef} style={styles.itemList} className="no-scrollbar">
        {order.items.map((item, idx) => (
          <div key={idx} style={styles.itemRow}>
            <div style={styles.qtyIndicator}>{item.quantity}</div>
            <div style={{ flex: 1 }}>
      ...
      {hasOverflow && (
        ...
        >
          ...
          <span style={{ marginLeft: 5 }}>SCROLL FOR MORE ITEMS</span>
        </motion.div>
      )}
...
const styles = {
  ...
  card: { background: '#0d0d0d', borderRadius: '28px', padding: '28px', display: 'flex', flexDirection: 'column', height: '420px', border: '1px solid', position: 'relative', overflow: 'hidden' },
  newRibbon: { position: 'absolute', top: 0, left: 0, right: 0, background: '#d3bfa2', color: '#000', fontSize: '0.65rem', fontWeight: 900, textAlign: 'center', padding: '6px', display: 'flex', alignItems: 'center', justifyContent: 'center', letterSpacing: '1px' },
  ...
  qtyIndicator: { width: '32px', height: '32px', background: 'rgba(211, 191, 162, 0.05)', border: '1px solid rgba(211, 191, 162, 0.1)', color: '#fff', display: 'flex', justifyContent: 'center', alignItems: 'center', borderRadius: '10px', fontWeight: 800, fontSize: '1rem', flexShrink: 0 },
